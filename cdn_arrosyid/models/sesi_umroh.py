...
class SesiUmroh(models.Model):
    ...
    pembimbing_id          = fields.Many2one(comodel_name='cdn.ustadz.pembimbing', string='Pembimbing', tracking=True)
    petugas_lapangan       = fields.Many2many(comodel_name='cdn.petugas.lapangan', string='Petugas Lapangan', tracking=True)
    jammaah_ids            = fields.One2many('cdn.pendaftaran', 'sesi_id', string='jammaah', tracking=True)
    jumlah_jamaah          = fields.Char(compute='_compute_jml_jamaah', string='Jumlah Jamaah', tracking=True)
    state                  = fields.Selection(string='Status', selection=[('akan_datang', 'Akan Datang'),('prosess', 'Sedang Berjalan'),('selesai', 'Selesai'),('batal_perjalanan', 'Perjalanan Batal')], default='akan_datang',required=True, tracking=True)
    itenerary              = fields.One2many('cdn.rencana.perjalanan', 'sesi_umroh_id', string='Itinerary')
    tanggal_berangkat      = fields.Date(string='Tanggal Berangkat', tracking=True)
